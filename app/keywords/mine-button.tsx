...
type Result = {
  ...
  thresholds: { STRIKING: { minImpressions: number; minPosition: number; maxPosition: number } };
};
    ...
    <div className={styles.block}>
      <div className={styles.row}>
        <div>
          ...
          <div className={styles.sub}>
            ...
            everything already targeted or previously rejected.
          </div>
        </div>
      ...
      {state === 'done' && result ? (
        <div className={styles.result}>
          ...
          {s ? ` (≥${s.minImpressions} impressions, position ${s.minPosition}–${s.maxPosition}, not already targeted, not previously rejected)` : ''}.
          {' '}
          ...
          {result.strikingCount === 0 ? (
            <p className={styles.note}>
              ...
              holds no non-brand rankings in the 11–20 band to promote.
            </p>
          ) : (
            <ul className={styles.list}>
              {result.striking.map((c) => (
                <li key={c.query}>
                  <span className={styles.mono}>{c.impressions} imp · pos {c.position}</span> {c.query}
                </li>
              ))}
            </ul>
          )}
